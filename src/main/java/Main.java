...
public class Main {
    private static final String endFlag = String.format("CHARSET(%s)*=(%s)*[\\d\\D]*?;",SEPERATOR,SEPERATOR);
    private static final String startFlag = String.format("CREATE(%s)*TABLE",SEPERATOR);
    private static Pattern endPattern = Pattern.compile(endFlag);
    private static Pattern startPattern = Pattern.compile(startFlag);
    ...
    public static void test(){
        String rowString = "`Fremark` varchar(516) NOT NULL COMMENT '流转意见内容',";
        DataType rowDataType = getRowDataType(rowString);
        Integer length = getLength(rowString, rowDataType);
        System.out.println(length);
        boolean nullAble = isNullAble(rowString);
        System.out.println(nullAble);
        System.out.println(getRowComment(rowString));
        System.out.println(getRowDefaultValue(rowString));
        String a = ") ENGINE=InnoDB AUTO_INCREMENT=168 DEFAULT CHARSET=utf8mb4 COMMENT='采购退货单SKU明细';";
        System.out.println(getAutoIncreament(a));
        System.out.println(getTableComment(a));
        System.out.println(getTableCharset(a));
        String b= "`Fdecimal` decimal(22,4 ) DEFAULT NULL,";
        System.out.println(getLength(b,DataType.DECIMAL));
        System.out.println(getDecimalLength(b,DataType.DECIMAL));
    }

    private static void handle() throws IOException {
        String fileName = "db.sql";
        String fileContent = getFileContent(fileName);
//        FileOutputStream fileOutputStream = new FileOutputStream("fileContent.txt");
...
//        outputStreamWriter.flush();
    ...
    private static String getFileContent(String fileName) throws IOException {
        ...
        try(InputStream resourceAsStream = Main.class.getClassLoader().getResourceAsStream("sql/"+fileName)) {
            int read = 1;
            assert resourceAsStream != null;
            while (read != -1){
                byte[] tempBytes = new byte[1024];
                read = resourceAsStream.read(tempBytes);
                content.append(new String(tempBytes, StandardCharsets.UTF_8));
            }
    ...
    private static Integer getLength(String rowString,DataType dataType){
        ...
        int pos = rowString.indexOf(jdbcName);
